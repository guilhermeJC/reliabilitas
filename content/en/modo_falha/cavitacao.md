...
plano_manutencao:
  - tarefa: 'Analyze high-frequency band vibration'
    ...
    duracao: '0.5 h per train'
    passos:
      - 'Confirm with operations that the pump is at stable load and record flow, suction/discharge pressures and fluid temperature'
      - 'Measure spectrum and envelope at the DE (drive-end) and NDE bearings, H/V/A directions, with the same collector and the same points as always (baseline repeatability)'
      - 'Read the 10–25 kHz band RMS and compare with the healthy-machine baseline at the SAME load condition'
      - 'Check for sub-synchronous components (0.3–0.8×RPM — recirculation) and elevated kurtosis in the high band'
      - 'Record numeric values and the boundary condition in the CMMS; update the trend'
    registros:
      - '10–25 kHz band RMS — DE bearing [g]'
      - '10–25 kHz band RMS — NDE bearing [g]'
      - 'Sub-synchronous amplitude 0.3–0.8×RPM [mm/s]'
      - 'Flow at measurement time [m³/h]'
  - tarefa: 'Inspect suction and volute acoustically'
    ...
    especialidade: 'Predictive / ultrasound'
    duracao: '0.25 h'
    passos:
      - 'Scan the suction line, nozzle and volute with the ultrasonic sensor at the same reference points as the baseline'
      - 'Characterize the noise: continuous gravel (cavitation) × irregular crackling (suspected air)'
      - 'If air is suspected: tightness test on suction connections/sealing'
      - 'Record dB levels and the characterization in the CMMS'
    registros:
      - 'Ultrasonic level — suction [dB]'
      - 'Ultrasonic level — volute [dB]'
  - tarefa: 'Monitor NPSH margin and BEP position'
    ...
    especialidade: 'Reliability engineering / process'
    duracao: 'Continuous; monthly trend review'
    passos:
      - 'Compute NPSHa from instrumentation (tank level/pressure, temperature → Pv, line losses)'
      - 'Compare with the manufacturer-curve NPSHr at current flow and check the normative margin'
      - 'Check the flow position relative to BEP (POR 70–120%)'
      - 'Review alarm thresholds whenever the reference operating point changes'
    registros:
      - 'Computed NPSHa [m]'
      - 'NPSHr at current flow [m]'
      - 'Flow as % of BEP [%]'
      - 'Fluid temperature [°C]'
  - tarefa: 'Inspect the impeller visually'
    ...
    duracao: '2 h (with lockout and draining)'
    passos:
      - 'Issue the Work Permit, apply lockout-tagout (LOTO), drain and depressurize the casing'
      - 'Insert the boroscope through the suction nozzle and inspect the low-pressure face of the vanes near the eye'
      - 'Photograph leading edges and pitted regions; compare with the previous inspection record'
      - 'Estimate pit depth and extent; classify against the repair criterion'
      - 'Record photos and measurements in the CMMS; if rejected, open a weld-repair + balancing work order'
    registros:
      - 'Estimated maximum pit depth [mm]'
      - 'Affected area extent [% of vane]'
tags:
  - NPSH
  - erosion
  - Rayleigh-Plesset
...
**What it is.** Inside a [[bomba-centrifuga|centrifugal pump]], pressure at the impeller inlet can drop below the liquid's **vapor pressure**. At that instant the liquid boils locally **from low pressure, not from heat**: every liquid boils when its pressure-temperature pair (P×T) crosses the saturation curve — ordinary kitchen boiling changes the temperature while holding pressure fixed; cavitation does the opposite, changing the pressure while holding temperature fixed. The physical result is the same — vapor bubbles form — but the trigger is the opposite one. Those bubbles, dragged into higher-pressure regions inside the impeller, **collapse violently** against metal surfaces.
