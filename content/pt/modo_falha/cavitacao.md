...
plano_manutencao:
  - tarefa: 'Analisar vibração em banda de alta frequência'
    metodo: 'Espectro + envelope; energia banda larga 10–25 kHz modulada por BPF; kurtosis na banda alta'
    periodicidade: 'Mensal (P-F/2)'
    condicao: 'Em operação, carga estável — registrar vazão e ponto de operação da medição'
    criterio: 'RMS da banda 10–25 kHz ≤ 2× o baseline da máquina sã; sem sub-síncronos 0,3–0,8×RPM'
    acao: 'Confirmar com ultrassom; auditar NPSHa e posição no BEP; corrigir pela camada operacional antes de intervir na bomba'
    especialidade: 'Preditiva / análise de vibração'
    duracao: '0,5 h por conjunto'
    passos:
      - 'Confirmar com a operação que a bomba está em carga estável e registrar vazão, pressões de sucção/descarga e temperatura do fluido'
      - 'Medir espectro e envelope nos mancais LA (lado acoplado) e LOA, direções H/V/A, com o mesmo coletor e os mesmos pontos de sempre (repetibilidade do baseline)'
      - 'Ler o RMS da banda 10–25 kHz e comparar com o baseline da máquina sã na MESMA condição de carga'
      - 'Verificar presença de componentes sub-síncronos (0,3–0,8×RPM — recirculação) e kurtosis elevada na banda alta'
      - 'Registrar valores numéricos e a condição de contorno no CMMS; atualizar a tendência'
    registros:
      - 'RMS banda 10–25 kHz — mancal LA [g]'
      - 'RMS banda 10–25 kHz — mancal LOA [g]'
      - 'Amplitude sub-síncrona 0,3–0,8×RPM [mm/s]'
      - 'Vazão no momento da medição [m³/h]'
  - tarefa: 'Inspecionar acusticamente a sucção e a voluta'
    metodo: 'Ultrassom/estetoscópio; ruído de "cascalho" contínuo; distinguir de entrada de ar (teste de estanqueidade)'
    periodicidade: 'Mensal, junto à rota de vibração'
    condicao: 'Em operação, junto à rota de vibração'
    criterio: 'Sem ruído de cascalho contínuo; nível ultrassônico estável frente ao baseline'
    acao: 'Ruído presente com NPSHa adequado → investigar recirculação e entrada de ar (tabela de diagnóstico diferencial)'
    especialidade: 'Preditiva / ultrassom'
    duracao: '0,25 h'
    passos:
      - 'Varrer com o sensor ultrassônico a linha de sucção, o bocal e a voluta, no mesmo ponto de referência do baseline'
      - 'Caracterizar o ruído: cascalho contínuo (cavitação) × estalos irregulares (suspeita de ar)'
      - 'Se houver suspeita de ar: teste de estanqueidade nas conexões/vedações da sucção'
      - 'Registrar os níveis em dB e a caracterização no CMMS'
    registros:
      - 'Nível ultrassônico — sucção [dB]'
      - 'Nível ultrassônico — voluta [dB]'
  - tarefa: 'Monitorar margem de NPSH e posição no BEP'
    ...
    condicao: 'Contínuo, em operação (PIMS)'
    criterio: 'NPSHa dentro da margem normativa (razão 1,1–2,5 — HI 9.6.1; ≥ NPSHr + 1 m — API 610); vazão no POR (70–120% do BEP)'
    acao: 'Margem violada → restaurar nível/pressão de sucção, reduzir vazão pela descarga ou reduzir rotação (NPSHr ∝ N²)'
    especialidade: 'Engenharia de confiabilidade / processo'
    duracao: 'Contínuo; revisão mensal da tendência'
    passos:
      - 'Calcular o NPSHa a partir da instrumentação (nível/pressão do tanque, temperatura → Pv, perdas da linha)'
      - 'Comparar com o NPSHr da curva do fabricante na vazão atual e verificar a margem normativa'
      - 'Verificar a posição da vazão em relação ao BEP (POR 70–120%)'
      - 'Revisar os limiares de alarme quando o ponto de operação mudar de referência'
    registros:
      - 'NPSHa calculado [m]'
      - 'NPSHr na vazão atual [m]'
      - 'Vazão em % do BEP [%]'
      - 'Temperatura do fluido [°C]'
  - tarefa: 'Inspecionar visualmente o impelidor'
    metodo: 'Boroscopia; morfologia dos pits (bordas rugosas, metal limpo) e profundidade vs critério de reparo'
    periodicidade: 'Oportunidade / anual'
    condicao: 'Parada de oportunidade, bomba drenada e bloqueada'
    criterio: 'Pits rasos e dispersos; borda de ataque íntegra; sem trincas coalescendo'
    acao: 'Coalescência ou borda comprometida → recuperar por solda com liga resistente + balancear; recorrência → avaliar upgrade de material'
    especialidade: 'Mecânica'
    duracao: '2 h (com bloqueio e drenagem)'
    passos:
      - 'Emitir PT, aplicar bloqueio e etiquetagem (LOTO), drenar e despressurizar a carcaça'
      - 'Inserir o boroscópio pelo bocal de sucção e inspecionar a face de baixa pressão das pás junto ao olho'
      - 'Fotografar as bordas de ataque e as regiões com pits; comparar com o registro da inspeção anterior'
      - 'Estimar profundidade e extensão dos pits; classificar contra o critério de reparo'
      - 'Registrar fotos e medidas no CMMS; se reprovado, abrir ordem de recuperação por solda + balanceamento'
    registros:
      - 'Profundidade máxima de pit estimada [mm]'
      - 'Extensão da área afetada [% da pá]'
tags:
  - NPSH
  - erosão
  - Rayleigh-Plesset
...
1. **Operacional imediata** (sem parar a máquina): restaurar nível/pressão de sucção; limpar filtros; reduzir vazão **pela válvula de descarga** — **nunca pela sucção** (estrangular a sucção derruba a pressão de entrada e intensifica o colapso; erro de operação clássico documentado por Perez, cap. 4) **e nunca abaixo da MCSF** da máquina: abaixo da vazão mínima contínua estável a recirculação de sucção passa a ser a fonte de dano, e o operador apenas troca um modo de falha por outro (ver [[bomba-centrifuga|curva H–Q, BEP e vazão mínima]]); reduzir rotação via VFD — pelas leis de similaridade o **NPSHr cai com o quadrado da rotação** (−20% de rotação ≈ −36% de NPSHr, com bônus de −49% de potência).
...
Tratando a erosão como desgaste: **β ≈ 1,5–3** sobre o tempo em condição cavitante — use a calculadora abaixo para sensibilidade de $R(t)$, MTTF e B10. No Fw A a categoria é **Mixed/Complex** (Nowlan & Heap: os padrões de gatilho aleatório dominam): não há idade característica de falha enquanto a condição operacional não se instala — **[[tbm|TBM]] é tecnicamente inválida** (trocar impelidor "a cada N meses" não remove nem antecipa o gatilho). Com P-F detectável e falha evidente, a decisão é **[[cbm|CBM]] com periodicidade P-F/2** (Moubray, cap. 7 — tarefas preditivas), monitoramento contínuo justificado em ativos críticos ou de P-F curto. A saída definitiva permanece operacional/de projeto: margem de NPSH e posição no BEP são variáveis de engenharia, não de manutenção.
